...
w.components['hint'] = {
	template: '<div class=def-hint-box v-bind:class=\"[ show ? \'show\' : \'\', landed ? \'def-hint-box_transitions\' : \'\' ]\" v-bind:style=\"{height:this.elHeight+\'px\'}\"><div class=def-hint-box__toggle v-on:click.prevent=toggle() ref=toggleButton>{{header}} <i class=toggle-icon><svg width=11px height=15px viewBox=\"0 0 11 15\" version=1.1 xmlns=http://www.w3.org/2000/svg xmlns:xlink=http://www.w3.org/1999/xlink><g id=close stroke=#BEBEBE stroke-width=1 stroke-linecap=round><path d=\"M1,1 L5.5,14\" stroke-linejoin=round></path><path d=\"M10,1 L5.5,14 L10,1 Z\" stroke-linejoin=round></path></g></svg></i></div><div class=def-hint-box__content ref=boxContent><slot></slot></div></div>',
	props: ['header'],
	data: function() {
...
(function(w){
"use strict";
// File /Users/john/rmsource/rmcode-help/_build/components/project-views.js

w.components['project-views'] = {
	template: '<div class=custom-widget-container><input type=range id=project-views-range v-on:input=setValue($event.currentTarget.value) v-bind:value=this.value class=additional-pvs min=0 max=39 step=1 v-on:mousedown=hasDrag v-on:mouseup=hasDrag><div class=\"montly-limit-section floating-section\"><div class=floating-section__amount v-bind:style=sliderPos><span class=amount__prefix>+</span> <span class=amount__number ref=variableViewsBlock>{{views}}</span> <span class=amount__description>project views<br>per month</span></div></div><div class=\"plan-description-section centered-section\"><div class=centered-section__header><div class=\"billing-switcher control-wrapper\"><span v-on:click.prevent=toggleSwitcher($event.currentTarget.dataset.type) class=\"billing-switcher__control billing-switcher__monthly\" data-type=month v-bind:class=\"[ this.price.selected == \'month\' ? \'selected\' : \'\']\">monthly billing </span><span v-on:click.prevent=toggleSwitcher($event.currentTarget.dataset.type) class=\"billing-switcher__control billing-switcher__annually\" data-type=year v-bind:class=\"[ this.price.selected == \'year\' ? \'selected\' : \'\']\">annual billing <span class=save-shield>Save 20%</span></span></div></div><div class=\"centered-section__amount column__right unit-price\"><span class=\"amount__number unit-price-month\" v-show=\"price.selected == \'month\'\">${{price.month}}</span> <span class=\"amount__number unit-price-year\" v-show=\"price.selected == \'year\'\">${{price.year}}</span> <span class=amount__addition><span class=hanging-element>+</span> current<br>plan price</span></div></div></div>',
	data: function () {
		return {
			value: 0,
			views: '10,000',
...
// End of /Users/john/rmsource/rmcode-help/_build/components/project-views.js
})(window);
(function(w){
"use strict";
